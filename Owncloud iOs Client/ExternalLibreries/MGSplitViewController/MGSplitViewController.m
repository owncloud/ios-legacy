...
//  MGSplitView
//
//  Created by Matt Gemmell on 26/07/2010 and modified by Gonzalo Gonzalez
...
#define MG_DEFAULT_CORNER_RADIUS_IOS7	0.0 	// default corner-radius of overlapping split-inner corners on the master and detail views.
...
- (void)layoutSubviewsForInterfaceOrientation:(UIInterfaceOrientation)theOrientation withAnimation:(BOOL)animate
{
	...
	if (!_cornerViews) {
        ...
        leadingCorners.cornerRadius = MG_DEFAULT_CORNER_RADIUS_IOS7;
        ...
        trailingCorners.cornerRadius = MG_DEFAULT_CORNER_RADIUS_IOS7;
...
{   
    ...
    //Add this method for ios6 but also its works in ios5
    if ([self isLandscape]) {        
        [self layoutSubviews];
    }
}
...
- (void)reconfigureForMasterInPopover:(BOOL)inPopover
{
	...
	if (inPopover && !_hiddenPopoverController && !_barButtonItem) {
        ...
        //Custom image for the owncloud app
        if(k_have_icon_on_popover) {
            UIImage *image = [UIImage imageNamed:@"RootFolderIcon.png"];
            _barButtonItem = [[UIBarButtonItem alloc] initWithImage:image style:UIBarButtonItemStyleBordered target:self action:@selector(showMasterPopover:)];
        } else {
            NSString *appName = [[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleDisplayName"];
            _barButtonItem = [[UIBarButtonItem alloc] initWithTitle:appName style:UIBarButtonItemStyleBordered target:self action:@selector(showMasterPopover:)];
        }
...
- (IBAction)showMasterPopover:(id)sender
{
	if (_hiddenPopoverController && !(_hiddenPopoverController.popoverVisible)) {
        ...
        //Dismiss popover when the user tap the button if popover is visible
...
- (void)setDividerStyle:(MGSplitViewDividerStyle)newStyle
{
	...
	if (_dividerStyle == MGSplitViewDividerStyleThin) {
         cornerRadius = MG_DEFAULT_CORNER_RADIUS_IOS7;
